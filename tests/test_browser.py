...
Tests ensure_chrome_running() delegation to ai-dev-browser's browser_start().
"""
...
class TestEnsureChromeRunning:
    ...
    @pytest.mark.asyncio
    async def test_delegates_to_browser_start(self):
        """ensure_chrome_running should call browser_start with correct args."""
        mock_result = {"port": 9350, "pid": 12345, "reused": False}
        with patch("grok_web.browser.browser_start", return_value=mock_result) as mock_start:
    ...
    async def test_passes_port_when_specified(self):
        """Should pass port to browser_start when specified."""
    ...
    async def test_custom_profile(self):
        """Should pass custom profile to browser_start."""
    ...
    async def test_raises_on_error(self):
        """Should raise RuntimeError when browser_start returns error."""
        mock_result = {"error": "No available port"}
        with patch("grok_web.browser.browser_start", return_value=mock_result):
