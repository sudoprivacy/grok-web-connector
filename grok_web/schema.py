...
PARAMS: dict[str, dict[str, Any]] = {
    "images": {
        "desc": (
            ...
            "(only for create_video → triggers video-extend), "
            "'file:<uuid>' (only for create_video → previously uploaded "
            "via client.upload_images, skips re-upload), or 'ref:<uuid>' "
            "(only for create_video → a saved reference from "
            "create_reference; keeps a character/subject consistent, "
            "in-Grok / low-moderation, no re-upload). \n\n"
            "**Per-method semantics**:\n"
            "  • create_video: post: → img2vid (single source); video: → "
            "video-extend (single source); ref: → reference-conditioned "
            "video (one or more; consistent character, NO starting image); "
            "file:/local paths → upload2vid with @N marker support.\n"
            "  • create_image: Grok-native 'post:<image_id>' (or bare "
            "image_id) refs are in-Grok img2img by id, NO re-upload (low mod). "
            "ONE ref = img2img from it; MULTIPLE all-Grok refs = compose them "
            "(needs a warmed session — do a create_video first, else raises a "
            "warming hint). Local files (or a Grok+local mix) upload as refs; "
            "@N in prompt maps to images[N-1]. Empty/omitted = txt2img. "
            "'ref:<id>' (saved Reference) is rejected — video-only.\n"
            "  • edit_image: images[0] is the post being edited (must be "
            "post:<uuid>); images[1:] are reference images. @1 = source, "
            "@2..@N+1 = refs. 'ref:<id>' is rejected — references are "
            "video-only.\n"
