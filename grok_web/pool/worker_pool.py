...
def matches_exception(result, cls: type) -> bool:
    """Return True iff the exception recorded on ``result`` was of type
    ``cls`` or a subclass of it.

    Reads ``result.error_type`` and ``result.error_bases`` (native
    fields on JobResult since ai-dev-browser 0.9.3, populated by the
    worker's catch via ``_handle_job_failure(..., exc=e)``). Operates
    on the serialized class names (not a real ``isinstance`` check
    because the exception object itself doesn't survive the
    ...
    Returns False when the result didn't come from an exception (e.g.
    a ``fail_condition`` soft failure has ``error_type=None``).
    ...
    Example::
        ...
        if matches_exception(result, GrokRateLimitError):
            # Catches both GrokRateLimitError and GrokQuotaExceededError.
            await asyncio.sleep(600)
    """
    cls_name = getattr(result, "error_type", None)
    if not cls_name:
        return False
    if cls_name == cls.__name__:
        return True
    bases = getattr(result, "error_bases", None) or []
    return cls.__name__ in bases
        ...
        while self._running and worker.status != WorkerStatus.STOPPING:
            try:
                ...
                try:
                    ...
                    # Pass the exception object through so ai-dev-browser
                    # 0.9.3+ populates JobResult.error_type / error_bases
                    # natively. Downstream callers branch via
                    # pool.matches_exception(result, GrokRateLimitError)
